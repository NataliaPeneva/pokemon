...
const rareSpawnChance = (pokemons) => {
  return pokemons.filter((pokemon) => pokemon.spawn_chance < 0.01)
}

console.log("rareSpawnChance " + rareSpawnChance(pokemons).length)
...
console.log("nextEvolutionPresent " + nextEvolutionPresent(pokemons).length)
...
const normalizedData = (pokemons) => {
  const normalData = pokemons.map((pokemon) => {
    ...
    pokemon.egg === "Not in Eggs" ? (pokemon.egg = 0) : (pokemon.egg = parseInt(pokemon.egg, 10) * 1000)
    return pokemon
  })
  return normalData
}
